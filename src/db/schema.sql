-- ============================================================
-- src/db/schema.sql
-- SQLite データベースのテーブル定義（将来の SQLite 移行時の参考）。
-- ============================================================
-- 現在の戦績記録は src/db/stats.js で JSON ファイル（./data/fever-mj.json）に
-- 保存している。これは Windows + Node 24 環境で better-sqlite3 のネイティブ
-- ビルドが通らない（Python 未インストール）ため、外部依存のない JSON 保存に
-- 切り替えた経緯による。
--
-- 本ファイルは将来 SQLite に移行する際の参考スキーマとして残してある。
-- 利用する場合は package.json の dependencies に better-sqlite3 を追加し、
-- ビルド環境（Python・Visual Studio Build Tools）を整備してから移行する。
-- ============================================================
